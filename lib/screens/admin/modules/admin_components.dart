          ...
          child: StreamBuilder<List<Map<String, dynamic>>>(
            ...
            builder: (_, snap) {
              ...
              return ListView.separated(
                ...
                itemBuilder: (_, i) => _CompRow(
                    comp: comps[i], service: service),
              );
            },
          ),
        ),
  ...
  void _showDialog(BuildContext context, {Map<String, dynamic>? existing}) {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text(existing == null ? 'New Component' : 'Edit Component',
...
class _CompRow extends StatelessWidget {
      ...
      child: Row(children: [
        ...
        Expanded(
          child: Column(
            ...
            children: [
              Text(comp['name']?.toString() ?? 'Unknown',
                  style: const TextStyle(
                      color: AppTheme.textPrimary,
                      fontWeight: FontWeight.w600, fontSize: 13)),
              Text('Type: ${comp['type']?.toString() ?? '—'}',
                  style: const TextStyle(
  ...
  void _showEditDialog(BuildContext context) {
    final nameCtrl =
        TextEditingController(text: comp['name']?.toString() ?? '');
    final typeCtrl =
        TextEditingController(text: comp['type']?.toString() ?? '');
    showDialog(
      context: context,
      builder: (_) => AlertDialog(
        backgroundColor: AppTheme.darkCard,
